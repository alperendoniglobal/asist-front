...
import { Loader2, ArrowRight, Phone, Package } from 'lucide-react';
...
export default function DeliveryReturnPage() {
    ...
    const fetchContent = async () => {
      try {
        ...
        const [content, landing] = await Promise.all([
          contentService.getPageBySlug('delivery-return').catch(() => null),
  ...
  return (
    <>
      <Helmet>
        <html lang="tr" />
        <title>{pageContent?.meta_title || 'Teslimat ve İade | Çözüm Asistan'}</title>
        ...
        <meta property="og:title" content={pageContent?.meta_title || 'Teslimat ve İade | Çözüm Asistan'} />
